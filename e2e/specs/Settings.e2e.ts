...
describe('Settings Screen', () => {
  ...
  it('should save language selections and persist them', async () => {
    ...
    if (await settingsIcon.isDisplayed()) {
      ...
      const headerIcons = await $$(
        '//android.view.View[@class="android.view.View" and @clickable="false"]'
      );
    ...
    if (checkboxes.length >= 3) {
      ...
      console.log(
        `State of first checkbox after clicking: ${await getCheckboxState(checkboxes[0])}`
      );
      console.log(
        `State of second checkbox after clicking: ${await getCheckboxState(checkboxes[1])}`
      );
      console.log(
        `State of third checkbox after clicking: ${await getCheckboxState(checkboxes[2])}`
      );
    } else {
    ...
    const saveButton = await $(
      '//android.view.ViewGroup[@content-desc="Save" and @resource-id="save-button"]'
    );
    ...
    if (checkboxes.length >= 3) {
      ...
      console.log(
        `Second checkbox state after returning: ${await getCheckboxState(checkboxes[1])}`
      );
      ...
      await browser.pause(5000);
      console.log(
        `First checkbox state after additional wait: ${await getCheckboxState(checkboxes[0])}`
      );
      console.log(
        `Second checkbox state after additional wait: ${await getCheckboxState(checkboxes[1])}`
      );
      console.log(
        `Third checkbox state after additional wait: ${await getCheckboxState(checkboxes[2])}`
      );
